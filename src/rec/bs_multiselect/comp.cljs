(ns rec.bs-multiselect.comp
  (:require-macros [reagent.ratom :refer [reaction]])
  (:require [reagent.core :as r :refer [atom]]
            [rec.bs-dropdown.comp :refer [bs-dropdown format-data find-by-id]]))

(defn- set-selection! [state item selected?]
  (swap! state
         assoc
         :data
         (map #(if (= item %)
                (assoc % :selected selected?)
                %)
              (:data @state))))

(defn bs-multiselect
  [{:keys [data selected placeholder on-change icon-class on-delete]
    :or {selected []
         data []
         placeholder "Type something"
         on-change identity}}]
  (let [formatted (map #(assoc % :selected false) (format-data data))
        state (atom {:data formatted
                     :focus false})
        selected (reaction (filter :selected (:data @state)))]
    (fn []
      [:div.bs-multiselect-container
       {:class (when (:focus @state) "active")
        :style {:margin-bottom :20px}}
       [:div.input-group
        {:style {:z-index :auto}}
        [:div.input-group-addon [:i {:class icon-class}]]
        [(bs-dropdown
           {:data (filter (comp not :selected) (:data @state))
            :placeholder placeholder
            :value ""
            :on-focus #(swap! state assoc :focus true)
            :on-blur #(swap! state assoc :focus false)
            :focus (:focus @state)
            :on-select (fn [_ item]
                         (set-selection! state item true)
                         (on-change @selected))})]
        [:div.input-group-addon.delete-button {:on-click on-delete} [:i.fa.fa-times]]]
       [:div.selected-container
        (for [item @selected]
          ^{:key (gensym)}
          [:div.selected-item
           {:on-mouse-down (fn []
                             (set-selection! state item false)
                             (on-change @selected))}
           [:i.zmdi.zmdi-delete]
           (when-let [cat (:category item)] [:span.cat (name cat)])
           [:span.name (:name item)]])]])))

#_(defn bs-multiselect2
  [{:keys [data selected placeholder on-change icon-class]
    :or {selected []
         placeholder "Type something"
         on-change identity}}]
  (let [formatted (map #(assoc % :selected false) (into {} (map (fn [[k v] [k (format-data v)]]) data)))
        state (atom {:data formatted
                     :focus false
                     :current-cat (first (keys data))})
        selected (reaction (filter :selected (:data @state)))]
    (fn []
      [:div.bs-multiselect-container
       {:class (when (:focus @state) "active")}
       [:div.input-group
        [:div.input-group-addon [:i {:class icon-class}]]
        [(bs-dropdown
           {:data (keys (:data @state))
            :placeholder placeholder
            :value ""
            :on-focus #(swap! state assoc :focus true)
            :on-blur #(swap! state assoc :focus false)
            :focus (:focus @state)
            :on-select (fn [_ item]
                         (swap! state assoc :current-cat item)
                         (on-change @selected))})]
        [(bs-dropdown
           {:data ((:current-cat @state) (:data @state))
            :placeholder placeholder
            :value ""
            :on-focus #(swap! state assoc :focus true)
            :on-blur #(swap! state assoc :focus false)
            :focus (:focus @state)
            :on-select (fn [_ item]
                         (set-selection! state item true)
                         (on-change @selected))})]]
       [:div.selected-container
        (for [item @selected]
          ^{:key (gensym)}
          [:div.selected-item
           {:on-mouse-down (fn []
                             (set-selection! state item false)
                             (on-change @selected))}
           [:i.zmdi.zmdi-delete]
           (when-let [cat (:category item)] [:span.cat (name cat)])
           [:span.name (:name item)]])]])))
